...
cout<<"2.Find student by name\n";
cout<<"3.Find and delete student by name\n";
cout<<"4.Jump to main\n";
cin>>choice;
...
case '3'://Display data
{  ifstream f2("student.txt");
   ofstream temp("temp_student.txt",ios::app);
 //  ofstream temp2("temp_student2.txt",ios::app);

cout<<"Enter student first name that you want to delete: ";
cin>>find;
cout<<endl;
int notFound = 0;
for( j=0;(j<i)||(!f2.eof());j++)
...
getline(f2,studentData.fname);
//shifting matching data to temp file
if(studentData.fname == find)
{
  notFound = 1;
 temp<<studentData.fname<<endl;
 getline(f2,studentData.lname);

 temp<<studentData.lname<<endl;
getline(f2,studentData.Registration);
 temp<<studentData.Registration<<endl;
 getline(f2,studentData.classes);
 temp<<studentData.classes<<endl;
...
ifstream temp9("temp_student.txt");
ifstream prevfile("student.txt");
string linesfromtemp9file;
string linesfromprevfile;
int successcompare ;

successcompare =  system("fc temp_student.txt student.txt > temp_student2.txt ");



if(successcompare){
	cout << "Student record deleted successfully !"<<endl;
	ifstream f98("temp_student2.txt");
	ofstream fnew("temp_student3.txt",ios::app);
	string purify;

	while (getline(f98,purify)){
		if(purify != "Comparing files temp_student.txt and STUDENT.TXT" && purify != "***** temp_student.txt" && purify	!= "***** STUDENT.TXT" && purify !="*****"){
			fnew << purify << endl;
		}
	}
	f98.close();
...
remove("temp_student2.txt");
remove("student.txt");
rename("temp_student3.txt","student.txt");
